...
size_t DirectReader::decodeNTR( FILE *fp, size_t offset,size_t offset2, size_t length,unsigned char *buf )
{
    fseek( fp, offset, SEEK_SET );
    int v = ((offset-offset2)*7+length*5)%127+11;
    int d = ((offset-offset2)*2+length*3)%488+536;
    if (length <= d) {
