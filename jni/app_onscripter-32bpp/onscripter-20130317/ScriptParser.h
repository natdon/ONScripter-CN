...
public:
    ...
    int intlimitCommand();
    int incCommand();
        int ifCommand(); // Modified by Taigacon
        int elseCommand(); //Added by Taigacon
    int humanzCommand();
    int gotoCommand();
    int gosubCommand();
    int globalonCommand();
    ...
    int addCommand();
        int switchCommand();//Added by Taigacon
        int caseCommand();//Added by Taigacon
        int defaultCommand();//Added by Taigacon
...
public:
    struct NestInfo{
		...
		bool is_func;	//added by Taigacon
		ScriptHandler::VariableData vd[16];  // To support local variable -- added by Taigacon
...
protected:
    ...
    bool condition_flag;  //ifCommand
    bool switch_flag; //switchCommand
    int z_order;
    bool rmode_flag;
...
public:
	bool is_user_func;	//added by Taigacon
	int localonCommand();	//added by Taigacon
    	void errorAndExit( const char *str, const char *reason=NULL );	//modified bt Gjy
