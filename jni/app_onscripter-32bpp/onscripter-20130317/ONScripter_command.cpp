...
void ONScripter::setwindowCore()
{
    ...
    if ( buf[0] == '#' ){
		...
		ai->orig_pos.w += (Uint16)(ai->orig_pos.w / 7.5);
		else if(screen_ratio1 == 1025 || screen_ratio1 == 1281)
		ai->orig_pos.w += (Uint16)(ai->orig_pos.w / 3.5);
		//printf("%d\n",ai->orig_pos.w);
...
int ONScripter::inputboxCommand()
{
#ifdef ANDROID
    script_h.readStr();
	...
	androidinpubox(" ‰»Î",buf);


    printf( "*** inputboxCommand(): $%d is set to the default value: %s\n",
            no, buf );
#endif
    return RET_CONTINUE;
}

int ONScripter::getinputextCommand()
{
#ifdef ANDROID
	script_h.readStr();
	if ( script_h.current_variable.type != ScriptHandler::VAR_STR ) 
        errorAndExit( "input: no string variable." );
    int no = script_h.current_variable.var_no;
	const char *buf=getEdittext();
	printf("%s",getEdittext());
	setStr( &script_h.getVariableData(no).str, buf );
	printf( "*** getinputCommand(): $%d is set to the default value: %s\n",
            no, buf );
#endif
	return RET_CONTINUE;
}
...
int ONScripter::gettagCommand()
{
    ...
    if (buf[0] == '[')
        buf++;
    else if (zenkakko_flag && buf[0] == 0x81 && buf[1] == 0x79)
        buf += 2;
    else
        buf = NULL;
    ...
    if ((*pretext_buf)[0] == ']')
        (*pretext_buf)++;
             ...
             (*pretext_buf)[0] == 0x81&& (*pretext_buf)[1] == 0x7a)
        *pretext_buf += 2;
...
int ONScripter::getspnumCommand()
{
    bool getsp2num_flag = false;
    if (script_h.isName("getsp2num")) getsp2num_flag = true;
    ...
    int spnum = 0;
    for (int i = 0;i < num; i++)
    spnum += si[i].file_name?1:0;
    script_h.setInt( &script_h.pushed_variable, spnum );
    return RET_CONTINUE;
}
...
int ONScripter::cleannumCommand()
{
    int no = script_h.readInt();
    bool range = false;
    int range_no = 0;
    if ( script_h.getEndStatus() & ScriptHandler::END_COMMA ) {
    range_no = script_h.readInt();
    range = true;
    }
    if (!range) 
        script_h.setNumVariable(no,0);
    else {
        for (int i = no;i <= range_no;i++)
            script_h.setNumVariable(i,0);
    }
...
int ONScripter::cleanstrCommand()
{
    int no = script_h.readInt();
    bool range = false;
    int range_no = 0;
    ...
    if (!range) 
        setStr( &script_h.getVariableData(no).str, NULL );
    else {
        for (int i = no;i <= range_no;i++)
            setStr( &script_h.getVariableData(i).str, NULL );
    }
...
int ONScripter::cspCommand()
{
    ...
    bool csp_range = false;
	
	int num_start = 0,num_end = num;
    int csp_range_num = 0;
    if ( script_h.getEndStatus() & ScriptHandler::END_COMMA ) {
        csp_range_num = script_h.readInt();
        csp_range = true;
    }
    ...
    if ( no == -1 || csp_range )
	{
        ...
        if ( csp_range ) {
            if ( no < 0 || csp_range_num >= num ) return RET_CONTINUE;
            num_start = no;
            num_end = csp_range_num + 1;
        }
...
int ONScripter::checkspCommand()
{
    script_h.readVariable();
    script_h.pushVariable();
    bool checksp2_flag = false;
    if (script_h.isName("checksp2")) checksp2_flag = true;
    int no = script_h.readInt();
    AnimationInfo *si = NULL;
    if (checksp2_flag) 
        si = sprite2_info;
        ...
        si = sprite_info;
    script_h.setInt( &script_h.pushed_variable, si[no].file_name?1:0);
    return RET_CONTINUE;
}
...
int ONScripter::ext_vCommand()
{
    char buf[256];
    if (script_h.getStringBuffer()[2] == '\0'){
        if ( wave_sample[0] ){
            Mix_Pause( 0 );
            Mix_FreeChunk( wave_sample[0] );
            wave_sample[0] = NULL;
        }
    }
    else {
        if (!defv_path) {
            defv_path = new char[8];
            sprintf(defv_path,RELATIVEPATH "voice%c", DELIMITER);
        }
        if (!defv_ext) defv_ext = "ogg";
        sprintf(buf, "%s%s.%s", defv_path,script_h.getStringBuffer()+2,defv_ext);
        playSound(buf, SOUND_CHUNK, false, 0);
    }
    return RET_CONTINUE;
}

int ONScripter::ext_bgmCommand()
{
    char buf[256];
    mp3stopCommand();
    stopBGM( false );
    if (script_h.getStringBuffer()[4] == '\0')
        return RET_CONTINUE;
    if (!defbgm_path) {
            defbgm_path = new char[6];
            sprintf(defbgm_path,RELATIVEPATH "bgm%c", DELIMITER);
    }
    if (!defbgm_ext) defbgm_ext = "ogg";
    sprintf(buf, "%s%s.%s", defbgm_path,script_h.getStringBuffer()+4,defbgm_ext);
...
int ONScripter::defvCommand()
{
    const char *buf1 = script_h.readStr();
    defpath(&defv_path,buf1);
    delete[] buf1;
    if ( script_h.getEndStatus() & ScriptHandler::END_COMMA ){
        const char *buf2 = script_h.readStr();
        defext(&defv_ext,buf2,"ogg");
        delete[] buf2;
    }
    return RET_CONTINUE;
}

int ONScripter::defbgmCommand()
{
    const char *buf1 = script_h.readStr();
    defpath(&defbgm_path,buf1);
    delete[] buf1;
    if ( script_h.getEndStatus() & ScriptHandler::END_COMMA ){
        const char *buf2 = script_h.readStr();
        defext(&defbgm_ext,buf2,"ogg");
        delete[] buf2;
    }
    return RET_CONTINUE;
}
...
int ONScripter::sprintfCommand(){
        ...
        if (*str == '\0') break;
            ...
            if (*str == '%')
				...
				while(!(toupper(ch)>='A' && toupper(ch)<='Z'))
					modebuf[s_len++]=ch,ch=*str++;
				modebuf[s_len++]=ch;
				modebuf[s_len++]='\0';
				char buf2[512];
				if(toupper(ch)=='S')
					snprintf(buf2,512,modebuf,script_h.readStr());
				else
					snprintf(buf2,512,modebuf,script_h.readInt());
				buf[buf_offset++]='\0';
				strcat(buf,buf2);
				buf_offset+=strlen(buf2)-1;
            }
        }
        else
            buf[buf_offset++] = *str++;
    ...
    if ( script_h.pushed_variable.type == ScriptHandler::VAR_INT ||
         script_h.pushed_variable.type == ScriptHandler::VAR_ARRAY ){
       errorAndExit("sprintf: Error type of variable.");
    }
    else if ( script_h.pushed_variable.type == ScriptHandler::VAR_STR ){
        int no = script_h.pushed_variable.var_no;
        setStr( &script_h.getVariableData(no).str, buf );
    }
    else errorAndExit( "sprintf: no variable." );
    return RET_CONTINUE;
}

int ONScripter::defpath(char **buf,const char *set_value)
{
    if (*buf) delete[] *buf;
    if (set_value[0] == '\0') {
        *buf = new char[2];
        ...
        *buf = new char[strlen(set_value)+3];
        sprintf(*buf,RELATIVEPATH "%s%c", set_value, DELIMITER);
    }
    return 0;
}

int ONScripter::defext(char **buf,const char *set_value,const char *default_value)
{
    if (*buf) delete[] *buf;
    if (set_value[0] == '\0') {
        *buf = new char[strlen(default_value)+1];
        strcpy(*buf,default_value);
    }
    else{
        *buf = new char[strlen(set_value)+1];
        strcpy(*buf,set_value);
    }
    return 0;
}
