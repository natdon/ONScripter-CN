...
int ONScripter::loadSaveFile2( int file_version )
{
    ...
    if (file_version >= 206){
        ...
        if (file_version >= 209) {  
            // Added by Taigacon , to support savegame in if/else and switch/case/default command.
            condition_flag = readChar();
            switch_flag = readChar();
        }
...
void ONScripter::saveSaveFile2( bool output_flag )
{
    ...
    writeInt( underline_value, output_flag ); // changed in version 207
            // Added by Taigacon , to support savegame in if/else and switch/case/default command.
            writeChar(condition_flag?1:0,output_flag);
            writeChar(switch_flag?1:0,output_flag);
