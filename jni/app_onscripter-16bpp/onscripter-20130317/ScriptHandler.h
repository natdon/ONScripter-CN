...
public:
	...
	int decodeNT3(char **buf,size_t size,FILE *fp);
	...
	VariableInfo pushed_variable;
	VariableInfo switch_buf; 
    char *save_path;
    int screen_width;
    int screen_height;
    int variable_range;
...
private:
    ...
    //added by taigacon
public:
    inline char getchar3(const char *buf){return *buf;};
    char *getptr(const char *str);
    inline void setchar(char *buf,char v){*buf=v;};
    inline char getchar2(char buf){return buf;};
	bool localon_flag;
	ScriptParser *sp;
};
