...
int NsaReader::open( char *nsa_path )
{
	...
	const char* xp3_name[8] = {"data","image","voice","fgimage","sound","bgm","patch","envimage"};
    ...
    if (num_of_ns2_archives ==0 && archive_type & ARCHIVE_TYPE_NSA){
        for ( i=0 ; i<8 ; i++){
           ArchiveInfo *ai;
           //sprintf( archive_name2, "patch%d", i+1 );
           sprintf( archive_name, "%s%s.%s", nsa_path, xp3_name[i], "xp3");
           ai = &archive_info2[num_of_nsa_archives];
            
        
            if ( ( ai->file_handle = fopen( archive_name, "rb" ) ) == NULL ) continue;
        ...
        for ( i=MAX_EXTRA_ARCHIVE ; i>-2 ; i-- ){
           ArchiveInfo *ai;
        
            if (i == -1){
                sprintf( archive_name, "%s%s.%s", nsa_path, "patch", nsa_archive_ext );
                ai = &archive_info2[num_of_nsa_archives];
            }
            else{
                //sprintf( archive_name2, "patch%d", i+1 );
                sprintf( archive_name, "%spatch%d.%s", nsa_path, i+1, nsa_archive_ext );
                ai = &archive_info2[num_of_nsa_archives];
            }
        ...
        for ( i=-1 ; i<MAX_EXTRA_ARCHIVE ; i++ ){
           ArchiveInfo *ai;
            ...
            if (i == -1){
                ...
                sprintf( archive_name, "%sarc%d.%s", nsa_path, i+1, nsa_archive_ext );
                ai = &archive_info2[num_of_nsa_archives];
            }
