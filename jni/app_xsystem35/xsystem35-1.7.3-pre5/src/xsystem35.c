...
void sys_message(char *format, ...) {
	...
	__android_log_vprint(ANDROID_LOG_INFO, "xsystem35", format, args);
...
void sys_error(char *format, ...) {
	...
	va_start(args, format);
	vfprintf(stderr, format, args);
	__android_log_vprint(ANDROID_LOG_ERROR, "xsystem35", format, args);
	va_end(args);
...
void sys_exit(int code) {
	...
	__android_log_print(ANDROID_LOG_INFO, "xsystem35", "sys_exit(%d)", code);
...
static void signal_handler(int sig_num) {
	...
	__android_log_print(ANDROID_LOG_ERROR, "xsystem35", "signal_handler(%d)", sig_num);
...
static void signal_handler_segv(int sig_num) {
	__android_log_print(ANDROID_LOG_ERROR, "xsystem35", "PID(%d), sigsegv caught @ %03d, %05x\n", (int)getpid(), sl_getPage(), sl_getIndex());
