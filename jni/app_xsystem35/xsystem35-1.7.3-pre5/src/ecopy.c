...
static void eCopyUpdateAreaWithoutRefresh(int sx, int sy, int w, int h, int dx, int dy) {
	MyRectangle src;
	MyPoint dst;
	...
	UpdateAreaWithoutRefresh(&src, &dst);
}
...
static int eCopyArea3(int dx, int dy, int w, int h, int opt) {
	...
	while (h > 0) {
		cnt += waitcnt;
		eCopyUpdateAreaWithoutRefresh(dx, y1, w, 1, dx, y1);
		eCopyUpdateAreaWithoutRefresh(dx, y2, w, 1, dx, y2);
		Refresh();
		y1 += 2;
		y2 -= 2;
		h -= 2;
...
static int eCopyArea4(int dx, int dy, int w, int h, int opt) {
	...
	while (w > 0) {
		cnt += waitcnt;
		eCopyUpdateAreaWithoutRefresh(x1, dy, 1, h, x1, dy);
		eCopyUpdateAreaWithoutRefresh(x2, dy, 1, h, x2, dy);
		Refresh();
		x1 += 2;
		x2 -= 2;
		w -= 2;
...
static int eCopyArea6(int dx, int dy, int w, int h, int opt) {
	...
	for (i = 0; i < 7; i++) {
		cnt += waitcnt;
		for (x = 0; x < (w -63); x += 64) {
			eCopyUpdateAreaWithoutRefresh(dx+x+i*4,     dy, 4, h, dx+x+i*4, dy);
			eCopyUpdateAreaWithoutRefresh(dx+x+(60-i*4),dy, 4, h, dx+x+(60-i*4), dy);
		}
		for (y = 0; y < (h -63); y += 64) {
			eCopyUpdateAreaWithoutRefresh(dx+4, dy+y+i*4,      w-8, 4, dx+4, dy+y+i*4);
			eCopyUpdateAreaWithoutRefresh(dx+4, dy+y+(60-i*4), w-8, 4, dx+4, dy+y+(60-i*4));
		}
		Refresh();
		EC_WAIT;
	}
...
static int eCopyArea7(int dx, int dy, int w, int h, int opt) {
	...
	for (i = 0; i < (w/2 - E7_8X+1); i+=E7_8X) {
		cnt += waitcnt;
		eCopyUpdateAreaWithoutRefresh(dx, dy+i*h/w, w, E7_8X*h/w, dx, dy+i*h/w);
		eCopyUpdateAreaWithoutRefresh(dx, dy+(h-(i+E7_8X)*h/w), w, E7_8X*h/w, dx, dy+(h-(i+E7_8X)*h/w));
		eCopyUpdateAreaWithoutRefresh(dx+i, dy, E7_8X, h, dx+i, dy);
		eCopyUpdateAreaWithoutRefresh(dx+(w-i-E7_8X), dy, E7_8X, h, dx+(w-i-E7_8X), dy);
		Refresh();
		EC_WAIT;
	}
	eCopyUpdateArea(dx, dy, w, h, dx, dy);
...
#define ECA11_SLICE 16
	...
	for (i = 0; i < ECA11_SLICE + h / ECA11_SLICE -1; i++) {
		...
		for (j = 0; j < min(i+1, ECA11_SLICE); j++) {
			y = j + ECA11_SLICE*(i-j);
			if (y < 0 || y >= h) continue;
			eCopyUpdateAreaWithoutRefresh(dx, dy + y, w, 1, dx, dy + y);
		}
		Refresh();
		EC_WAIT;
	}
...
static int eCopyArea16(int dx, int dy, int w, int h, int opt) {
	...
	for (i = 0; i < 8; i++) {
		cnt += waitcnt;
		for (x = 0; x < (w -15); x += 16) {
			eCopyUpdateAreaWithoutRefresh(dx + x + (7-i), dy, 1, h, dx + x + (7-i), dy);
			eCopyUpdateAreaWithoutRefresh(dx + x + (8+i), dy, 1, h, dx + x + (8+i), dy);
		}
		Refresh();
		EC_WAIT;
	}
...
#define E17X 18
	...
	for (i = 0; i < E17X; i++) {
		...
		for (y = 0; y < (h - E17X + 1); y += E17X) {
			eCopyUpdateAreaWithoutRefresh(dx, dy + y + i, w, 1, dx, dy + y + i);
		}
		Refresh();
		EC_WAIT;
	}
...
static int eCopyArea18(int dx, int dy, int w, int h, int opt) {
	...
	for (x = 0; x < (w/2 -7); x += 8) {
		cnt += waitcnt;
		eCopyUpdateAreaWithoutRefresh(dx + w/2 - x - 8, dy, 8, h, dx + w/2 - x - 8, dy);
		eCopyUpdateAreaWithoutRefresh(dx + w/2 + x    , dy, 8, h, dx + w/2 + x    , dy);
		Refresh();
		EC_WAIT;
	}
	eCopyUpdateArea(dx, dy, w, h, dx, dy);
...
static int eCopyArea19(int dx, int dy, int w, int h, int opt) {
	...
	for (x = 0; x < (w/2 -7); x += 8) {
		cnt += waitcnt;
		eCopyUpdateAreaWithoutRefresh(dx + x,         dy, 8, h, dx + x,         dy);
		eCopyUpdateAreaWithoutRefresh(dx + w - x - 8, dy, 8, h, dx + w - x - 8, dy);
		Refresh();
		EC_WAIT;
	}
	eCopyUpdateArea(dx, dy, w, h, dx, dy);
...
static int eCopyArea20(int dx, int dy, int w, int h, int opt) {
	...
	for (y = 0; y < (h/2 -7); y += 8) {
		cnt += waitcnt;
		eCopyUpdateAreaWithoutRefresh(dx, dy + h/2 - y - 8, w, 8, dx, dy + h/2 - y - 8);
		eCopyUpdateAreaWithoutRefresh(dx, dy + h/2 + y    , w, 8, dx, dy + h/2 + y    );
		Refresh();
		EC_WAIT;
	}
	eCopyUpdateArea(dx, dy, w, h, dx, dy);
...
static int eCopyArea21(int dx, int dy, int w, int h, int opt) {
	...
	for (y = 0; y < (h/2 -7); y += 8) {
		cnt += waitcnt;
		eCopyUpdateAreaWithoutRefresh(dx, dy + y        , w, 8, dx, dy + y);
		eCopyUpdateAreaWithoutRefresh(dx, dy + h - y - 8, w, 8, dx, dy + h - y - 8);
		Refresh();
		EC_WAIT;
	}
	eCopyUpdateArea(dx, dy, w, h, dx, dy);
...
static int eCopyArea33(int dx, int dy, int w, int h, int opt) {
	...
	for (i = 0; i < ECA11_SLICE + h / ECA11_SLICE -1; i++) {
		...
		for (j = 0; j < min(i+1, ECA11_SLICE); j++) {
			y = j + ECA11_SLICE*(i-j);
			if (y < 0 || y >= h) continue;
			eCopyUpdateAreaWithoutRefresh(dx, dyy - y, w, 1, dx, dyy - y);
		}
		Refresh();
		EC_WAIT;
	}
...
static int eCopyArea39(int sx, int sy, int w, int h, int dx, int dy, int opt) {
	...
	for (i = 0; i < h2; i++) {
		cnt+=waitcnt;
		eCopyUpdateAreaWithoutRefresh(sx, sy1, w, 1, dx, dy1);
		eCopyUpdateAreaWithoutRefresh(sx, sy2, w, 1, dx, dy2);
		Refresh();
		dy1++;
		sy1++;
		dy2--;
...
static int eCopyArea40(int sx, int sy, int w, int h, int dx, int dy, int opt) {
	...
	for (i = 0; i < w2; i++) {
		cnt+=waitcnt;
		eCopyUpdateAreaWithoutRefresh( sx1, sy, 1, h, dx1, dy);
		eCopyUpdateAreaWithoutRefresh( sx2, sy, 1, h, dx2, dy);
		Refresh();
		dx1++;
		sx1++;
		dx2--;
...
static int eCopyArea48(int dx, int dy, int w, int h, int opt) {
	...
	for (i = 0; i < ECA11_SLICE + w / ECA11_SLICE -1; i++) {
		...
		for (j = 0; j < min(i+1, ECA11_SLICE); j++) {
			x = j + ECA11_SLICE*(i-j);
			if (x < 0 || x >= w) continue;
			eCopyUpdateAreaWithoutRefresh( dx+x, dy, 1, h, dx+x, dy);
		}
		Refresh();
		EC_WAIT;
	}
...
static int eCopyArea49(int dx, int dy, int w, int h, int opt) {
	...
	for (i = 0; i < ECA11_SLICE + w / ECA11_SLICE -1; i++) {
		...
		for (j = 0; j < min(i+1, ECA11_SLICE); j++) {
			x = j + ECA11_SLICE*(i-j);
			if (x < 0 || x >= w) continue;
			eCopyUpdateAreaWithoutRefresh(dxx - x, dy, 1, h, dxx - x, dy);
		}
		Refresh();
		EC_WAIT;
	}
