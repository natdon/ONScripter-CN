...
static SDL_VideoInfo _vinfo;
static SDL_PixelFormat _vfmt;
...
static void window_init(void) {
	...
	_vfmt  = *_vinfo.vfmt;
	if(getenv("8BPP") != NULL){
		_vfmt.BitsPerPixel  = 8;
		_vfmt.BytesPerPixel = 1;
		_vfmt.Rmask = 0;
		_vfmt.Gmask = 0;
		_vfmt.Bmask = 0;
		_vfmt.Amask = 0;
	...
	sdl_vflag = SDL_SWSURFACE;//SDL_HWSURFACE | SDL_ANYFORMAT;
...
static void makeDIB(int width, int height, int depth) {
	...
	if (depth == 8) {
		...
		sdl_dib = SDL_AllocSurface(SDL_HWSURFACE, width, height, sdl_display->format->BitsPerPixel, 0, 0, 0, 0);
		//sdl_dib = SDL_AllocSurface(SDL_SWSURFACE/*SDL_HWSURFACE*/, width, height, sdl_display->format->BitsPerPixel, 0x7C00, 0x03E0, 0x001F, 0);
		//sdl_dib = SDL_AllocSurface(SDL_SWSURFACE/*SDL_HWSURFACE*/, width, height, sdl_display->format->BitsPerPixel, 0xF800, 0x07E0, 0x001F, 0);
	}
#else
	if(depth == 8){
		sdl_dib = SDL_AllocSurface(SDL_SWSURFACE, width, height, 8, 0, 0, 0, 0);
	} else {
		sdl_dib = SDL_AllocSurface(SDL_SWSURFACE, width, height, sdl_display->format->BitsPerPixel, sdl_display->format->Rmask, sdl_display->format->Gmask, sdl_display->format->Bmask, sdl_display->format->Amask);
	}
#endif
