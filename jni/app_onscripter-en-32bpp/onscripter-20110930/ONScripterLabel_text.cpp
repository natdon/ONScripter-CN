...
#define isRotationRequired(x) (onsLocaleIsRotationRequired((const unsigned char *)(x)))
#define isTranslationRequired(x) (onsLocaleIsTranslationRequired((const unsigned char *)(x)))
...
void ONScripterLabel::drawGlyph( SDL_Surface *dst_surface, Fontinfo *info, SDL_Color &color, char* text, int xy[2], bool shadow_flag, AnimationInfo *cache_info, SDL_Rect *clip, SDL_Rect &dst_rect )
{
    ...
    if (IS_TWO_BYTE(text[0])){
        ...
        if(encrypt)
		{
		index = ons_get_unencryption_short(index);
		*(unsigned short*)conv2 = index;
        	unicode = onsLocaleConv(((unsigned char)conv2[0]<<8)|(unsigned char)conv2[1]);
       ...
       // unicode = convSJIS2UTF16( ((unsigned char*)text)[0] );
        ...
        else if (text[0] == '\t') unicode = ' '; // draw tabs as spaces
        else unicode = text[0];
    }
                                ...
                                AnimationInfo *cache_info, int abs_offset, SDL_Rect *clip )
{
	...
	if(RetUseColor())
	{
		sentence_font.color[0] = Retfc1();
		sentence_font.color[1] = Retfc2();
		sentence_font.color[2] = Retfc3();
	}
#endif
