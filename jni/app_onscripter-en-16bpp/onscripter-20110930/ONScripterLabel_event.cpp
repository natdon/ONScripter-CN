        ...
        if (!useescspc_flag && (event->keysym.sym == SDLK_ESCAPE)){
            ...
            if (rmode_flag && (event_mode & WAIT_TEXT_MODE)){
                if (root_rmenu_link.next)
					...
					int screenshot_w = screen_width;
					int screenshot_h = screen_height;
					...
					#ifdef BPP16
						screenshot_surface = SDL_CreateRGBSurface( SDL_SWSURFACE, screenshot_w, screenshot_h, 32, 0xf800, 0x07e0, 0x001f, 0 );
					#else
						#ifdef ANDROID
						 screenshot_surface = SDL_CreateRGBSurface( SDL_SWSURFACE, screenshot_w, screenshot_h, 32,  0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000 );
						 #else
						 screenshot_surface = SDL_CreateRGBSurface( SDL_SWSURFACE, screenshot_w, screenshot_h, 32,  0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000 );
						 #endif
					#endif
				...
				if (wide_screen_mode){
					...
					int screenshot_w = swidth;
					int screenshot_h = swidth*3/4;
